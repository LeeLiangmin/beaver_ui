...
import { Plus, Trash2, Pencil, FolderOpen, Folder, File, Zap, Search, ChevronDown, ExternalLink, ArrowRight, AlertTriangle, LayoutGrid, Inbox } from 'lucide-react'
import type { OpenerGroup, OpenerItem } from '../../shared/types'

const COLORS: Record<string, string> = {
  teal: '#4DB6AC', cyan: '#26A69A', gold: '#FFD54F', pink: '#F8BBD0',
  green: '#80CBC4', coral: '#EF9A9A', lavender: '#CE93D8', orange: '#FFAB91',
}
...
export default function FastOpener() {
  ...
  useEffect(() => {
    const onClick = () => { setMoveMenuId(null); setShowAddMenu(false) }
  ...
  const openItem = async (item: OpenerItem) => {
    ...
    setItems((prev) => prev.map((i) =>
      i.id === item.id ? { ...i, useCount: i.useCount + 1, lastUsed: Math.floor(Date.now() / 1000) } : i
    ))
  }
  ...
  if (filterText) {
    ...
    filteredItems = filteredItems.filter(i => i.name.toLowerCase().includes(kw) || i.path.toLowerCase().includes(kw))
  }
  switch (sortBy) {
    case 'name': filteredItems.sort((a, b) => a.name.localeCompare(b.name)); break
    case 'useCount': filteredItems.sort((a, b) => b.useCount - a.useCount); break
    case 'lastUsed': filteredItems.sort((a, b) => b.lastUsed - a.lastUsed); break
    default: filteredItems.sort((a, b) => a.sortOrder - b.sortOrder)
  }
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 mb-3">
        <div className="relative">
          <button onClick={(e) => { e.stopPropagation(); setShowAddMenu(!showAddMenu) }}
            ...
            <Plus size={13} />添加<ChevronDown size={15} />
          </button>
          {showAddMenu && (
            <div className="absolute top-full left-0 mt-1 bg-white rounded-xl shadow-elevated border border-gray-200 py-1 z-30 min-w-[120px]">
              <button onClick={addFile} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-gray-700 hover:bg-primary-light hover:text-primary transition-colors">
                <File size={14} className="text-primary" />文件
              </button>
              <button onClick={addDir} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-gray-700 hover:bg-amber-50 hover:text-amber-600 transition-colors">
                <Folder size={14} className="text-amber-500" />目录
              </button>
            </div>
          )}
        </div>
        ...
        <div className="relative w-52">
          ...
          <input type="text" value={filterText} onChange={(e) => setFilterText(e.target.value)}
            placeholder="搜索名称或路径…" className="w-full border border-gray-300 rounded-lg pl-8 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-light focus:border-primary transition-colors" />
        </div>
        ...
        <aside className="w-40 shrink-0 bg-white rounded-lg border border-gray-200 shadow-card flex flex-col">
          <div className="flex-1 overflow-y-auto py-1">
            <button onClick={() => setActiveGroupId(null)}
              ...
              <LayoutGrid size={12} /><span className="flex-1 text-left truncate">全部</span>
            ...
            {groups.map((g) => (
              <div key={g.id}
                ...
                <span className="w-2.5 h-2.5 rounded-sm shrink-0" style={{ background: COLORS[g.color] || g.color }} />
                ...
                <div className="hidden group-hover/item:flex gap-0.5 shrink-0">
                  <button onClick={(e) => { e.stopPropagation(); setEditingGroup(g); setGroupForm({ name: g.name, color: g.color }); setShowGroupModal(true) }}
                    className="p-0.5 rounded text-gray-400 hover:text-primary transition-colors"><Pencil size={14} /></button>
                  <button onClick={(e) => { e.stopPropagation(); removeGroup(g.id) }}
                    className="p-0.5 rounded text-gray-400 hover:text-red-500 transition-colors"><Trash2 size={14} /></button>
                </div>
              </div>
            ))}
            <button onClick={() => setActiveGroupId('__ungrouped__')}
              ...
              <Inbox size={12} /><span className="flex-1 text-left truncate">未分组</span>
              <span className="text-gray-400">{groupItemCount('')}</span>
            </button>
          </div>
          <div className="border-t border-gray-200 p-2">
            <button onClick={() => { setEditingGroup(null); setGroupForm({ name: '', color: 'blue' }); setShowGroupModal(true) }}
          ...
          {filteredItems.length > 0 ? (
            <div className="grid gap-2" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))' }}>
              {filteredItems.map((item) => (
                <div key={item.id} onDoubleClick={() => openItem(item)}
                  draggable
                  onDragStart={() => handleDragStart(item.id)}
                  onDragOver={handleDragOver}
                  onDrop={() => handleDrop(item.id)}
                  className={`relative bg-white rounded-lg border p-3 transition-all cursor-pointer group/card shadow-card ${
                    dragId === item.id ? 'opacity-50 border-dashed border-primary' : 'border-gray-200 hover:border-primary/40 hover:shadow-elevated'
                  }`}>
                  {invalidPaths.has(item.id) && (
                    <div className="absolute top-2 right-2 flex items-center gap-1 px-1.5 py-0.5 text-xs text-red-500 bg-red-50 border border-red-200 rounded">
                      <AlertTriangle size={14} />失效
                    </div>
                  )}
                  <div className="flex items-start gap-3">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 border ${item.isDir ? 'bg-amber-50 text-amber-500 border-amber-200' : 'bg-primary-light text-primary border-primary/20'}`}>
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        {getGroup(item.groupId) && (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs border"
                            style={{ background: (COLORS[getGroup(item.groupId)!.color] || '#6366f1') + '15', color: COLORS[getGroup(item.groupId)!.color] || '#6366f1', borderColor: (COLORS[getGroup(item.groupId)!.color] || '#6366f1') + '30' }}>
                            <span className="w-1 h-1 rounded-sm" style={{ background: COLORS[getGroup(item.groupId)!.color] || '#6366f1' }} />
                            {getGroup(item.groupId)!.name}
                          </span>
                        )}
                        {item.useCount > 0 && <span className="text-xs text-gray-400">×{item.useCount}</span>}
                        {item.lastUsed > 0 && <span className="text-xs text-gray-400">{formatRelative(item.lastUsed)}</span>}
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-1 mt-2.5 pt-2 border-t border-gray-100 opacity-0 group-hover/card:opacity-100 transition-opacity">
                    <button onClick={(e) => { e.stopPropagation(); openItem(item) }}
                    ...
                    <button onClick={(e) => { e.stopPropagation(); openLocation(item.path) }}
                    ...
                    {groups.length > 0 && (
                      <div className="relative">
                        <button onClick={(e) => { e.stopPropagation(); setMoveMenuId(moveMenuId === item.id ? null : item.id) }}
                        ...
                        {moveMenuId === item.id && (
                          <div className="absolute bottom-full left-0 mb-1 bg-white rounded-xl shadow-elevated border border-gray-200 py-1 z-20 min-w-[120px]">
                            <button onClick={(e) => { e.stopPropagation(); moveToGroup(item.id, '') }}
                              className="w-full text-left px-3 py-1.5 text-xs text-gray-500 hover:bg-gray-50">未分组</button>
                            {groups.map((g) => (
                              <button key={g.id} onClick={(e) => { e.stopPropagation(); moveToGroup(item.id, g.id) }}
                                className="w-full text-left px-3 py-1.5 text-xs text-gray-500 hover:bg-gray-50 flex items-center gap-1.5">
                                <span className="w-1.5 h-1.5 rounded-sm shrink-0" style={{ background: COLORS[g.color] || g.color }} />{g.name}
                              </button>
                            ))}
                          </div>
                        )}
            ...
            <div className="flex flex-col items-center justify-center h-full gap-2">
              ...
              <p className="text-xs text-gray-400">{filterText ? '没有匹配的项目' : '添加常用文件和目录，一键快速打开'}</p>
            </div>
          )}
        </div>
      </div>
      ...
      {showGroupModal && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-[2px] z-50 flex items-center justify-center animate-fade-in" onClick={() => setShowGroupModal(false)}>
          <div className="bg-white rounded-2xl w-80 p-5 shadow-modal animate-modal-in" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-sm font-semibold text-gray-700 mb-4">{editingGroup ? '编辑分组' : '新建分组'}</h3>
            <div className="mb-3">
              ...
              <input type="text" value={groupForm.name} onChange={(e) => setGroupForm(f => ({ ...f, name: e.target.value }))}
                ...
                placeholder="输入分组名称" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-light focus:border-primary transition-colors" autoFocus />
            </div>
            <div className="mb-4">
              <label className="block text-xs text-gray-400 mb-1.5">分组颜色</label>
              <div className="flex gap-2 flex-wrap">
                {Object.entries(COLORS).map(([name, hex]) => (
                  <button key={name} onClick={() => setGroupForm(f => ({ ...f, color: name }))}
            ...
            <div className="flex gap-2 justify-end">
              <button onClick={() => setShowGroupModal(false)} className="px-4 py-2 text-sm text-gray-500 hover:bg-gray-100 rounded-lg transition-colors">取消</button>
