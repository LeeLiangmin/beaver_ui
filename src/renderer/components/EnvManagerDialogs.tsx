...
export function EnvGroupDialog({ editingGroup, groupForm, onClose, onChange, onSave }: GroupDialogProps) {
  return (
    <div
      ...
      onClick={onClose}
    >
      <div className="bg-white rounded-2xl w-80 p-5 shadow-modal animate-modal-in">
...
export function EnvAddDialog({
  ...
  return (
    <div
      ...
      onClick={onClose}
    >
      <div className="bg-white rounded-2xl p-6 w-[28rem] shadow-modal animate-modal-in">
...
export function EnvViewDialog({ item, onClose }: ViewDialogProps) {
  ...
  return (
    <div
      ...
      onClick={onClose}
    >
      <div className="bg-white rounded-2xl p-6 w-[36rem] max-h-[80vh] flex flex-col shadow-modal animate-modal-in">
        ...
        <div className="flex-1 min-h-0 border border-gray-200 rounded-lg overflow-hidden flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-2">
            {items.length === 0 ? (
              ...
              <div className="p-3 text-sm text-gray-700 break-all bg-gray-50 rounded-lg">
                {item.value || '(空值)'}
              </div>
            ) : (
              <div className="flex flex-col gap-1">
                {items.map((val, idx) => (
                  <div key={idx} className="flex items-start gap-2 px-3 py-2 bg-gray-50 rounded-lg">
                    <span className="shrink-0 w-5 h-5 flex items-center justify-center bg-gray-200 text-gray-500 text-[10px] font-medium rounded">
                      {idx + 1}
                    </span>
...
export function EnvEditDialog({
  ...
  return (
    <div
      ...
      onClick={onClose}
    >
      <div className="bg-white rounded-2xl p-6 w-[40rem] max-h-[85vh] flex flex-col shadow-modal animate-modal-in">
        <div className="flex items-center gap-3 mb-4">
            ...
            <div className="flex-1 min-h-0 overflow-y-auto border border-gray-200 rounded-lg p-2 space-y-2">
              {editItems.map((val, idx) => (
                <div key={idx} className="flex items-center gap-2 group">
                  <span className="shrink-0 w-6 h-6 flex items-center justify-center bg-gray-100 text-gray-500 text-[10px] font-medium rounded">
                    {idx + 1}
                  </span>
                  <input
                    type="text"
...
export function EnvDeleteDialog({ item, onClose, onConfirm }: DeleteDialogProps) {
  return (
    <div
      ...
      onClick={onClose}
    >
      <div className="bg-white rounded-2xl p-6 w-[22rem] shadow-modal animate-modal-in">
