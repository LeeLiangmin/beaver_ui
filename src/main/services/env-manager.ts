...
export function createGroup(name: string, color: string): EnvGroup {
  const db = getDb()
  const maxOrder = (db.prepare('SELECT MAX(sort_order) as m FROM env_groups').get() as any)?.m || 0
...
export function updateGroup(group: EnvGroup): void {
  const db = getDb()
  db.prepare('UPDATE env_groups SET name=?, color=?, sort_order=? WHERE id=?').run(
    group.name,
    group.color,
    group.sortOrder,
    group.id,
...
async function readRegistryEnv(root: 'HKLM' | 'HKCU'): Promise<Map<string, string>> {
  ...
  const hiveRoot =
    root === 'HKLM'
      ...
      : 'HKCU\\Environment'
  try {
    const { stdout } = await execFileAsync('reg', ['query', hiveRoot], {
      timeout: 5000,
    })
    for (const line of stdout.split('\n')) {
      const match = line.trim().match(/^\s*(.+?)\s+(REG_SZ|REG_EXPAND_SZ)\s+(.+)$/)
...
export async function getEnvVars(): Promise<EnvVar[]> {
  ...
  const db = getDb()
  const dbEntries = db.prepare('SELECT key, value, group_id FROM env_entries').all() as any[]

  const dbMap = new Map<string, string>()
  for (const e of dbEntries) {
    dbMap.set(e.key, e.group_id || '')
  }

  const result: EnvVar[] = []
  for (const [key, value] of merged) {
    result.push({ key, value, groupId: dbMap.get(key) || '' })
  }
...
export async function setEnvVar(key: string, value: string): Promise<void> {
  ...
  if (process.platform === 'win32') {
    await execFileAsync('setx', [k, value], { timeout: 5000 })
  }
  process.env[k] = value
  // Save to DB
  const db = getDb()
    ...
    for (const hive of ['HKCU', 'HKLM']) {
      const hiveRoot =
        hive === 'HKLM'
          ...
          : 'HKCU\\Environment'
      try {
        await execFileAsync('reg', ['delete', hiveRoot, '/v', k, '/f'], {
          timeout: 5000,
        })
      } catch {}
    }
...
export async function setEnvVars(items: EnvVar[]): Promise<{ success: number; failed: number }> {
  let success = 0,
    failed = 0
...
export function moveToGroup(key: string, groupId: string): void {
  ...
  db.prepare('INSERT OR REPLACE INTO env_entries (key, value, group_id) VALUES (?, ?, ?)').run(
    key,
    currentValue,
    groupId,
  )
...
export function removeFromGroup(key: string, groupId: string): void {
  const db = getDb()
  // 只删除该变量在指定分组中的备份快照
  db.prepare('DELETE FROM env_group_entries WHERE key = ? AND group_id = ?').run(key, groupId)
  // 检查该变量是否还属于其他分组
  const remaining = db
    .prepare('SELECT group_id FROM env_group_entries WHERE key = ? LIMIT 1')
    .get(key) as { group_id: string } | undefined
  // 更新主列表显示：有剩余分组则显示第一个，无则清空
  db.prepare('UPDATE env_entries SET group_id = ? WHERE key = ?').run(
    remaining?.group_id || '',
    key,
  )
}
...
export function getGroupEntries(groupId: string): EnvVar[] {
  const db = getDb()
  return db
    .prepare('SELECT key, value, group_id as groupId FROM env_group_entries WHERE group_id = ?')
    .all(groupId) as EnvVar[]
}

// ── 显式备份/恢复（操作 env_group_entries）──────────────────

export async function backupGroup(groupId: string): Promise<{ backedUp: number }> {
  const db = getDb()
  // 从备份表获取该分组实际包含的 key（支持多分组）
  const keys = db.prepare('SELECT key FROM env_group_entries WHERE group_id = ?').all(groupId) as {
    key: string
  }[]
  for (const { key } of keys) {
    // 读取当前实时值来更新快照
    const entry = db.prepare('SELECT value FROM env_entries WHERE key = ?').get(key) as
...
export async function backupGroupItem(groupId: string, key: string): Promise<void> {
  const db = getDb()
  const entry = db
    .prepare('SELECT key, value FROM env_entries WHERE group_id = ? AND key = ?')
...
export async function restoreGroup(groupId: string): Promise<{ restored: number }> {
  const db = getDb()
  const items = db
...
export async function restoreGroupItem(groupId: string, key: string): Promise<void> {
  const db = getDb()
  const backup = db
    .prepare('SELECT key, value FROM env_group_entries WHERE group_id = ? AND key = ?')
    .get(groupId, key) as { key: string; value: string } | undefined
...
export function escapeToml(s: string): string {
  return s
    .replace(/\\/g, '\\\\')
    .replace(/"/g, '\\"')
    .replace(/\n/g, '\\n')
...
export function unescapeToml(s: string): string {
  return s
    .replace(/\\t/g, '\t')
    .replace(/\\r/g, '\r')
    .replace(/\\n/g, '\n')
...
export function buildBackupToml(items: EnvVar[]): string {
...
export function parseBackupToml(content: string): EnvVar[] {
...
export async function backupEnvVars(targetDir?: string): Promise<string> {
  ...
  fs.writeFileSync(path.join(absDir, MERGED_FILE), buildBackupToml(items), 'utf-8')
  return absDir
}

export async function backupEnvVar(key: string, targetDir?: string): Promise<string> {
  const k = key.trim()
  if (!k) throw new Error('环境变量名不能为空')
...
export async function listEnvBackups(targetDir?: string): Promise<EnvBackupMeta[]> {
  const absDir = path.resolve(targetDir || getBackupDir())
  const fp = path.join(absDir, MERGED_FILE)
...
export async function restoreEnvVarsFromBackup(req: RestoreEnvRequest): Promise<RestoreEnvResult> {
...
export function deleteBackupItem(filePath: string, key: string): void {
  if (!filePath.trim()) throw new Error('备份文件路径不能为空')
  const items = parseBackupToml(fs.readFileSync(filePath, 'utf-8')).filter((it) => it.key !== key)
